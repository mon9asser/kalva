 ...
 * QueryBuilder
 *
 * Chainable MySQL query builder. Mirrors the PostgreSQL PgModel API.
 * Uses ? positional placeholders (mysql2 driver standard).
 *
 * @example
...
class QueryBuilder {
    ...
    /** Collect all WHERE bindings. Returns { sql, bindings }. */
    _buildWhere() {
        if (!this._wheres.length) return { sql: '', bindings: [] };
        const parts    = [];
        const bindings = [];
    ...
    _buildHaving() {
        if (!this._havings.length) return { sql: '', bindings: [] };
        const parts    = [];
        const bindings = [];

        for (const h of this._havings) {
            parts.push(h.sql);
            bindings.push(...h.bindings);
        }

        return { sql: ' HAVING ' + parts.join(' AND '), bindings };
    }
    ...
    _compile() {
        ...
        const { sql: whereSql,  bindings: whereB  } = this._buildWhere();
        const { sql: havingSql, bindings: havingB } = this._buildHaving();
    ...
    /**
     * WHERE col <op> value
     * Supported: =, !=, <>, <, >, <=, >=, LIKE, NOT LIKE, REGEXP, etc.
     */
    ...
    /**
     * WHERE col LIKE pattern — case-insensitive.
     * MySQL LIKE is case-insensitive by default for utf8mb4_unicode_ci.
     * For explicit case-insensitivity, wraps with LOWER().
     */
    whereILike(col, pattern) {
        return this._pushWhere('AND', `LOWER(${this._fqt}.\`${col}\`) LIKE LOWER(?)`, [pattern]);
    }

    /**
     * Raw WHERE fragment — use ? placeholders.
     * @param {string} sql
     * @param {any[]}  bindings
     */
    ...
    /**
     * Execute and return all matching rows as MySqlModel instances.
     * @returns {Promise<MySqlModel[]>}
     */
    async get() {
     ...
     * @param {number}   size
     * @param {Function} callback  - receives MySqlModel[] chunk
     */
    ...
    async sum(col)  { return this._runAggregate('SUM', col); }
    async avg(col)  { return this._runAggregate('AVG', col); }
    async min(col)  { return this._runAggregate('MIN', col); }
    async max(col)  { return this._runAggregate('MAX', col); }
    ...
    async increment(col, amount = 1) {
        this._applyGlobalScopes();
        const { sql: whereSql, bindings } = this._buildWhere();
        const sql = `UPDATE ${this._fqt} SET \`${col}\` = \`${col}\` + ?${whereSql}`;
    ...
    /** Execute a raw SQL string with ? params. */
...
class MySqlModel {
    ...
    /**
     * INSERT a row and return hydrated MySqlModel instance.
     * Uses insertId from the result to fetch the created row.
     * @param {object} data
     */
    static async create(data) {
        const result = await this._insertRaw(data);
    ...
    /**
     * INSERT OR IGNORE via INSERT IGNORE.
    ...
    static async createOrIgnore(data) {
        ...
        const sql  = `INSERT IGNORE INTO ${this._fqt} (${cols.map(c => `\`${c}\``).join(', ')}) VALUES (${ph})`;
        const result = await this._db().query(sql, vals);
        if (result.is_error || result.data?.affectedRows === 0) return null;
        return this.find(result.data.insertId);
    }

    /**
     * INSERT multiple rows. Returns MySqlModel[].
     * @param {object[]} rows
     */
     ...
     * @param {object} conditions  - columns used as unique/pk lookup
     * @param {object} data        - columns to set on update
     */
    static async updateOrCreate(conditions, data) {
        ...
        const ph         = cols.map(() => '?').join(', ');
        const updateCols = Object.keys(data).map(c => `\`${c}\` = VALUES(\`${c}\`)`).join(', ');

        const sql = `INSERT INTO ${this._fqt} (${cols.map(c => `\`${c}\``).join(', ')}) VALUES (${ph}) ON DUPLICATE KEY UPDATE ${updateCols}`;

        const result = await this._db().query(sql, vals);
        if (result.is_error) throw new Error(result.message);

        // insertId is the new id on INSERT, or the existing id on UPDATE (via LAST_INSERT_ID())
        const id = result.data.insertId || (await this.findOne(conditions))?.[this.primaryKey];
        return this.find(id);
    }
    ...
    static async update(id, data) {
        ...
        const vals = Object.values(data);
        const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
        const sql  = `UPDATE ${this._fqt} SET ${sets} WHERE \`${this.primaryKey}\` = ?`;
        const result = await this._db().query(sql, [...vals, id]);
        if (result.is_error) throw new Error(result.message);
        return result.data?.affectedRows ? this.find(id) : null;
    }
    ...
    static async delete(id) {
        const sql = `DELETE FROM ${this._fqt} WHERE \`${this.primaryKey}\` = ?`;
    ...
    static async truncate() {
        return this._db().query(`TRUNCATE TABLE ${this._fqt}`);
    }
     ...
     * Execute raw SQL.
     * @param {string} sql
     * @param {any[]}  bindings  - positional ? params
     */
     ...
     * @param {boolean} [cascade=false]  - MySQL ignores CASCADE on DROP TABLE;
     *                                     set to true to disable FK checks first.
     */
    static async rollbackMigration(cascade = false) {
        const db = this._db();
        if (cascade) await db.query('SET FOREIGN_KEY_CHECKS = 0');
        const result = await db.query(`DROP TABLE IF EXISTS ${this._fqt}`);
        if (cascade) await db.query('SET FOREIGN_KEY_CHECKS = 1');
     ...
     *
     * @param {typeof MySqlModel} RelatedModel
    ...
    async attach(pivotTable, localFK, foreignFK, ids, extraData = []) {
        ...
        return Promise.all(ids.map((id, i) => {
            ...
            const ph   = cols.map(() => '?').join(', ');
            const sql  = `INSERT IGNORE INTO \`${pivotTable}\` (${cols.map(c => `\`${c}\``).join(', ')}) VALUES (${ph})`;
    ...
    async detach(pivotTable, localFK, foreignFK, ids) {
        ...
        if (!ids || !ids.length) {
            return new Database().query(
                `DELETE FROM \`${pivotTable}\` WHERE \`${localFK}\` = ?`,
                [this[lk]]
            );
        }
        const ph  = ids.map(() => '?').join(', ');
        return new Database().query(
            `DELETE FROM \`${pivotTable}\` WHERE \`${localFK}\` = ? AND \`${foreignFK}\` IN (${ph})`,
            [this[lk], ...ids]
        );
    }
    ...
    static async _insertRaw(data) {
        const cols = Object.keys(data);
        const vals = Object.values(data);
        const ph   = cols.map(() => '?').join(', ');
        const sql  = `INSERT INTO ${this._fqt} (${cols.map(c => `\`${c}\``).join(', ')}) VALUES (${ph})`;
        return this._db().query(sql, vals);
    }
}

module.exports = { MySqlModel, QueryBuilder };